...
class SymptomCheckerModel:
    ...
    def _prepare_input(self, symptoms: List[SymptomInput], patient_info: Optional[PatientInfo]) -> List[float]:
        """Prepare input data for prediction. Ignores age/gender if not provided."""
        input_data = {}
        # Only use symptoms for feature vector
        input_data["num_symptoms"] = len(symptoms)
        # Remove age/gender from input if not present in feature_columns
        if "age" in self.feature_columns:
            input_data["age"] = patient_info.age if patient_info and patient_info.age else 0
        if "gender" in self.feature_columns:
            gender_val = patient_info.gender if patient_info and patient_info.gender else "unknown"
            if "gender" in self.label_encoders:
                try:
                    input_data["gender"] = self.label_encoders["gender"].transform([gender_val])[0]
                except Exception:
                    input_data["gender"] = 0
            else:
                input_data["gender"] = 0
