...
from app.schemas.ml_models import (
    ...
    SymptomCheckerFeedback, MLModelInfo, ModelTrainingRequest, SimpleSymptomInput
)
...
@router.post("/symptom-checker/simple", response_model=SymptomCheckerResult)
async def simple_symptom_checker(
    symptom_data: SimpleSymptomInput,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
):
    """Predict conditions from a simple list of symptoms (no age/gender required)"""
    try:
        ml_model = get_symptom_checker_model()
        # Convert symptom strings to SymptomInput with default severity
        ...
        session = SymptomChecker(
            ...
            initial_symptoms=[{"symptom": s.symptom, "severity": s.severity} for s in symptoms],
        ...
        logger.error(f"Error in simple symptom checker: {str(e)}")
