...
class SymptomCheckResult(BaseModel):
    ...
    is_chat: bool = Field(default=False, description="True when the response should be rendered as a normal chat reply")
    ...
    reply: Optional[str] = Field(default=None, description="Plain chat reply for frontend chat bubbles")
...
class SymptomChatResponse(BaseModel):
    ...
    is_chat: bool = Field(default=True, description="True when the response should be rendered as a normal chat reply")
